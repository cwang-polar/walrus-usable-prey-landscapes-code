...
**Hidden collapse in foraging capacity: Sea ice loss and Arctic shipping reshape Pacific walrus usable prey landscapes**
...
Data files are not included in this peer-review repository. Original public data sources are cited in the manuscript and Appendix S1. Processed input tables, derived gridded products, and model outputs will be archived in a permanent repository upon manuscript acceptance.
...
The peer-review code repository does not include input data files. See `data_availability_note.md` for the data-access plan and `figure_table_code_map.md` for how analysis code maps to manuscript outputs.

## Figure and table index

Every manuscript figure and appendix table is mapped to a reviewer-facing code
entry point in `figure_table_code_map.md`. The same output index is also stored
as `MANUSCRIPT_OUTPUT_CODE_INDEX` in `scripts/06_make_figures_and_tables.py`.
Figures 1 and 2 are explicitly marked as non-Python outputs.

## Verification

See `VERIFY.md` for checks that can be run on the repository code package. Full reproduction of manuscript results requires processed input data and external source products that are not included in this peer-review repository.
